...
ntpleaps_read(void)
{
	...
	u_int64_t *l;
	char *unixroot;
	char zonefile[CCHMAXPATH+1];

	unixroot = getenv("UNIXROOT");
	if (unixroot != NULL)
		strlcpy(zonefile, unixroot, CCHMAXPATH+1);
	else
		strlcpy(zonefile, "", CCHMAXPATH+1);

	strlcat(zonefile, "/usr/share/zoneinfo/right/UTC", CCHMAXPATH+1);

	fd = open(zonefile, O_RDONLY | O_NDELAY | O_BINARY);
	if (fd == -1)
		return (-1);
